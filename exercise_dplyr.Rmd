...
```{r}
# by using the pipe, you don't need to type "gapminder" as the first argument in filter()
gapminder |> 
...
```{r}
# remember to use "" around the country names!
gapminder |>
...
```{r}
# it's a good idea to separate the "or" function from the "and" function, so R knows how to group items and the order to perform functions in
gapminder |>
  filter((country == "China" | country == "Brazil") & pop > 100000000)

# to figure out if it is "any of these possible values" you use %in% to see if China or Brazil is present in the dataset.
# using c makes a vector that you can then compare to the dataset.
gapminder |>
  filter (country %in% c("China", "Brazil") & pop > 100000000)

# you can also assign the object to a variable name and use it that way!
allowed_countries <- c("China", "Brazil")
gapminder |>
  filter (country %in% allowed_countries & pop > 100000000)

# or you can filter out invalid cases like this!
invalid_cases <- c(10021)
filter(dataframe, ! case_id %in% invalid_cases)
```
...
```{r}
# notice how this creates a new column at the end!
# you can mutate multiple variables in the same call, as seen below
gapminder |>
  mutate(
    grossGDP = gdpPercap * pop,
    gdpBill = round(grossGDP / 1000000000, 2)
  )
# now, gdpBill is rounded to two decimals and displaying the dollar amount in billions
```
...
gapminder |> 
  ...
  summarize(mu = mean(lifeExp),
            sigma = sd(lifeExp))
# this gives one row for every group, so 60 groups makes 60 rows
# four columns for continent, year, mu (mean), and sigma (sd)
```
